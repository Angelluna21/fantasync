...
if (isset($_POST['guardar_receta'])) {
    ...
    // Obtener todos los ingredientes que actualmente tiene la receta con sus cantidades
    $ingredientes_actuales = [];
    $result = $conn->query("SELECT id_ingrediente, cantidad_por_base FROM receta WHERE id_platillo=$id_platillo");
    while ($row = $result->fetch_assoc()) {
        $ingredientes_actuales[(int)$row['id_ingrediente']] = (float)$row['cantidad_por_base'];
    }
    
    // Obtener todos los ingredientes disponibles para asegurar que procesamos todos los campos del formulario
    $todos_ingredientes = [];
    $result_all = $conn->query("SELECT id_ingrediente FROM ingrediente");
    while ($row = $result_all->fetch_assoc()) {
        $todos_ingredientes[] = (int)$row['id_ingrediente'];
    }
    
    // Procesar ingredientes del formulario
    // Estrategia: Solo actualizar los campos que tienen un valor > 0
    // Mantener todos los ingredientes existentes que no se modifiquen
    foreach ($todos_ingredientes as $id_ing) {
        // Si viene en el formulario con un valor
        if (isset($cantidades[$id_ing])) {
            $cantidad_str = trim($cantidades[$id_ing]);
            ...
            if ($cantidad_str !== '') {
                ...
                if ($cantidad > 0) {
                    ...
                    $sql = "INSERT INTO receta (id_platillo,id_ingrediente,cantidad_por_base)
                            VALUES ($id_platillo,$id_ing,$cantidad)
                            ON DUPLICATE KEY UPDATE cantidad_por_base=VALUES(cantidad_por_base)";
                    $conn->query($sql);
                } else if ($cantidad == 0 && isset($ingredientes_actuales[$id_ing])) {
                    // Si el usuario explícitamente puso 0 y el ingrediente existía, eliminarlo
                    $conn->query("DELETE FROM receta WHERE id_platillo=$id_platillo AND id_ingrediente=$id_ing");
                }
                // Si el campo está vacío (cadena vacía), no hacer nada (mantener valor existente si existe)
            }
            // Si el campo viene vacío, no hacer nada (mantener ingrediente existente)
        }
        // Si no viene en el formulario, no hacer nada (mantener ingrediente existente)
    }
    
    // Redirigir para limpiar el parámetro edit
...
if (isset($_GET['delete'])) {
    ...
    $conn->query("DELETE FROM receta WHERE id_platillo=$plat");
...
if ($editar_id > 0) {
    $platillo_editando = $conn->query("SELECT * FROM platillo WHERE id_platillo=$editar_id")->fetch_assoc();
    if ($platillo_editando) {

        // 🔥 CORREGIDO: orden y casting para evitar que falten valores
        $receta_data = $conn->query("
            SELECT id_ingrediente, cantidad_por_base 
            FROM receta 
            WHERE id_platillo=$editar_id
            ORDER BY id_ingrediente ASC
        ");
        while ($row = $receta_data->fetch_assoc()) {
            $id_ing = (int)$row['id_ingrediente'];
            $receta_actual[$id_ing] = $row['cantidad_por_base'];
        }
    }
}

// Listados
$platillos = $conn->query("SELECT * FROM platillo ORDER BY id_platillo ASC");
$ingredientes = $conn->query("SELECT * FROM ingrediente ORDER BY id_ingrediente ASC");
...
$sqlRec = "
    SELECT p.id_platillo,p.nombre AS platillo,p.porciones_base,
           GROUP_CONCAT(CONCAT(i.nombre,' (',r.cantidad_por_base,' ',i.unidad,')') SEPARATOR ', ') AS ingredientes
    FROM receta r
...
if ($search !== '') {
    ...
    $sqlRec .= "
      WHERE p.nombre LIKE '%$searchEsc%'
         OR i.nombre LIKE '%$searchEsc%'
    ";
}

$sqlRec .= "
    GROUP BY p.id_platillo
    ORDER BY p.id_platillo ASC
";

$recetas = $conn->query($sqlRec);

/* ====== 🔢 DATOS PARA LA CALCULADORA (SIN API, TODO AQUÍ) ====== */
$calcData = [];
$resCalc = $conn->query("
...
<div class="container">
  ...
  <div class="page-content">
    ...
    <div class="card">
      <!-- 🔥 C A L C U L A D O R A   D E   I N G R E D I E N T E S -->
      <div class="card" style="border: 2px solid #667eea; background:#eef2ff;">
        ...
        <div class="form-group">
          ...
          <button type="button" id="btn_calcular" class="btn primary" style="margin-top:10px;">
      ...
      <form method="post">
        <div class="form-group">
          ...
          <select name="id_platillo" required <?= $editar_id > 0 ? 'disabled' : '' ?>>
            ...
            $platillos->data_seek(0);
            while($p=$platillos->fetch_assoc()): 
              $selected = ($editar_id > 0 && $p['id_platillo'] == $editar_id) ? 'selected' : '';
            ?>
              <option value="<?=$p['id_platillo']?>" <?=$selected?>><?=h($p['nombre'])?> (Base: <?= (int)$p['porciones_base']?> porciones)</option>
            <?php endwhile; ?>
          </select>
          <?php if ($editar_id > 0): ?>
        ...
        <table id="tabla_ingredientes">
          <thead>
            <tr><th>Ingrediente</th><th>Unidad</th><th>Cantidad base</th></tr>
          </thead>
          <tbody>
            <?php 
            $ingredientes->data_seek(0); 
            while($i=$ingredientes->fetch_assoc()):
              $id_ing = (int)$i['id_ingrediente'];
              $valor_actual = $receta_actual[$id_ing] ?? '';
            ?>
              <tr>
                <td class="ing-nombre"><?=h($i['nombre'])?></td>
                <td><?=h($i['unidad'])?></td>
                <td><input type="number" step="0.001" name="cantidades[<?=$id_ing?>]" value="<?=$valor_actual?>"></td>
              </tr>
            <?php endwhile; ?>
          </tbody>
        </table>
    ...
    <div class="card" id="todas-las-recetas">
      ...
      <table id="tabla-recetas">
        <thead>
          <tr><th>Platillo</th><th>Porciones base</th><th>Ingredientes</th><th>Acción</th></tr>
        </thead>
        <tbody>
...
// Scroll automático a la tabla de recetas cuando hay una búsqueda
(function(){
  ...
  if (buscar && buscar.trim() !== '') {
    ...
    window.addEventListener('load', function(){
      ...
      if (tablaRecetas) {
        // Hacer scroll suave hasta la tabla
        setTimeout(function(){
          tablaRecetas.scrollIntoView({ behavior: 'smooth', block: 'start' });
          // Opcional: resaltar la tabla brevemente
...
document.addEventListener("DOMContentLoaded", ()=>{
  ...
  // --- Opción 2: Convertir kg pequeños a gramos ---
  if (r.unidad && r.unidad.toLowerCase() === 'kg') {
  ...
  // Ajuste para que pizza y gelatina sean piezas
  ...
  if (nombrePlatillo.includes('pizza') || nombrePlatillo.includes('gelatina')) {
    requeridoNum = Math.ceil(porciones); // 1 porción = 1 pieza
    baseCant = 1; // Mostrar base como 1 pieza
    r.unidad = 'pieza';
  }

  const requerido = requeridoNum.toFixed(0); // sin decimales

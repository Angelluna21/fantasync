<?php
// ingredientes.php - Versión PDO para SQLite
require_once 'db.php';
...
if (isset($_POST['add_ingrediente'])) {
  try {
    $stmt = $pdo->prepare("INSERT INTO ingrediente (nombre, unidad, tamanio_presentacion, presentacion_descripcion) 
...
if (isset($_GET['delete'])) {
  try {
    ...
    $stmt = $pdo->prepare("DELETE FROM ingrediente WHERE id_ingrediente = :id");
    $stmt->execute([':id' => $id]);
...
if ($search !== '') {
  $sqlIng .= " WHERE nombre LIKE :search 
                 OR unidad LIKE :search 
                 OR tamanio_presentacion LIKE :search 
                 OR presentacion_descripcion LIKE :search";
